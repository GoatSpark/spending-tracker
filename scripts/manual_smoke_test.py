...
.env credentials and writes real rows to the live DB. A bot can't fake an
inbound message to itself (sendMessage only sends bot -> user, it never
reaches the message handler), so this can only verify the bot is reachable
and prompt you for a manual round trip.
...
def main():
    ...
    payload = json.dumps({
        ...
        "text": "Smoke test: if you see this, the bot can send messages. "
                "Now send a real message like '$5 coffee' from your phone "
