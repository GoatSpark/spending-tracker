...
from stats import build_daily_totals, range_stats, format_cents, RANGE_DAYS


def _add_expense(db, cents, dt, note="test", raw="test"):
    expense = Expense(amount_cents=cents, note=note, timestamp=dt, raw_message=raw)
    db.add(expense)
    db.commit()
    return expense
...
def test_build_daily_totals_sums_same_day_entries(db_session):
    day = date(2026, 8, 1)
    _add_expense(db_session, 1200, datetime(2026, 8, 1, 12, 0))
    _add_expense(db_session, 899, datetime(2026, 8, 1, 18, 0))
